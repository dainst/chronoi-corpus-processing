...
regex_text_surrounded_by_whitespace_left = re.compile("^\s{2,}(([^\s]+ )+)\s{2,}")
regex_text_preceded_by_whitespace_right = re.compile("\s{4,}(([^\s]* ?)+)")
...
def line_delete_if_whitespace_exceeds(line, min_whitespace_ratio = 0.7):
    ...
    result = line
    if len(line) != 0:
        num_whitespace_chars = len(re.compile("\s").findall(line))
